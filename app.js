...
function getCovidSummary(country, ress) {
  ...
  const req = https.request(options, function (res) {
    ...
    res.on("end", function (chunk) {
      ...
      holidaySummary(country, data[2], ress);  //call holiday api
      console.log(data[2])
    });
...
function htmlDoc(holiday, data) {
  ...
  let holidayData;
  if(holiday && holiday[2]) {
    holidayData = `<p>Name: ${holiday[2].name}</p>
    <p>Description: ${holiday[2].description}</p>
    <p>Date: ${JSON.stringify(holiday[2].date)}</p>`;
  } else {
  ...
  <body>
      <h1>Covid data and a holiday to visit</h1>
